...
class TestWordpressK8s(unittest.TestCase):
    ...
    def test_use_nginx_ingress_modsec_change(self):
        """
        arrange: after peer relation established and database ready
        act: update use_nginx_ingress_modsec configuration
        assert: ingress with modsec should update according to the config
        """
        self._setup_replica_consensus()

        self.assertEqual(
            self.harness.charm.ingress_config["owasp-modsecurity-crs"],
            True,
            "use_nginx_ingress_modsec should be True by default",
        )

        self.harness.update_config({"use_nginx_ingress_modsec": False})

        self.assertEqual(
            self.harness.charm.ingress_config["owasp-modsecurity-crs"],
            False,
            "use_nginx_ingress_modsec should be False after update",
        )
