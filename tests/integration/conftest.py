...
async def prepare_mysql(wordpress: WordpressApp, model: Model):
    ...
    await model.wait_for_idle(status="active", apps=["mysql-k8s", wordpress.name], timeout=30 * 60)
...
async def prepare_loki(wordpress: WordpressApp, prepare_mysql):
    ...
    await wordpress.model.wait_for_idle(
        apps=[loki.name, wordpress.name], status="active", timeout=30 * 60, idle_period=60
    )
